        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    ...
                    'Pour rejoindre le MonsterQuest, vous avez besoin de Java 8. Voulez-vous installer une version de Java? En installant, vous inst <a href="http://www.oracle.com/technetwork/java/javase/terms/license/index.html">Oracle\'s license agreement</a>.',
                    'Install Java',
                    'Install Manually'
                )
                setOverlayHandler(() => {
...
function dlAsync(login = true){
    ...
    if(login) {
        if(ConfigManager.getSelectedAccount() == null){
            loggerLanding.error('Vous devez être connecter à un compte.')
            return
        }
    }
            ...
            if(login && allGood) {
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Explore les terres obscures')
                    } else if(GAME_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Se prépare à conquérir le monde')
                    }
                }
                ...
                const gameErrorListener = function(data){
                    ...
                    if(data.indexOf('Could not find or load main class net.minecraft.launchwrapper.Launch') > -1){
                        ...
                        showLaunchFailure('Error During Launch', 'The main file, LaunchWrapper, failed to download properly. As a result, the game cannot launch.<br><br>To fix this issue, temporarily turn off your antivirus software and launch the game again.<br><br>If you have time, please submit an issue</a> and let us know what antivirus software you use. We\'ll contact them and try to straighten things out.')
                    }
                }
                ...
                try {
                    ...
                    setLaunchDetails('Lancé. Profitez du jeu')
...
function setNewsLoading(val){
    if(val){
        const nLStr = 'Récupération des News'
        let dotStr = '..'
