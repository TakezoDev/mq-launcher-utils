    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `Echec de la suppression<br>Drop-in Mod ${fullName}`,
                    'Soyez sûr que le fichier est disponible.',
                    'Okay'
                )
        ...
        if(dropinUI != null){
            ...
            if(DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled){
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            "Echec de l'activation<br>One or More Drop-in Mods",
                            err.message,
                            'Okay'
                        )
                        setOverlayHandler(null)
...
function populateReleaseNotes(){
    $.ajax({
        ...
        settingsAboutChangelogText.innerHTML = 'Récupération impossible des notes.'
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        settingsUpdateButtonStatus('Récupérer des MAJ', false, () => {
            if(!isDev){
                ipcRenderer.send('autoUpdateAction', 'checkForUpdate')
                settingsUpdateButtonStatus('Récupération des MAJ..', true)
            }
        })
    }
}
